...
def parse_frontmatter(text):
  """Return (dict_or_None, ok).

  ok is False when a frontmatter block is present but unparseable, or absent
  entirely.
  """
...
def main(bundle):
  ...
  if errors:
    print(
        f"\nFAIL: {len(concepts) if isinstance(concepts, list) else concepts}"
        f" concepts, {len(errors)} error(s), {len(warnings)} warning(s)"
    )
    return 1

  print(
      f"\nPASS: {concepts} concepts, 0 frontmatter errors, {len(warnings)}"
      " broken-link warning(s)"
  )
  return 0
