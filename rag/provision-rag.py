...
Usage:
    # 1. Provision GCS bucket, upload handbook.pdf, and create Vertex AI Search
    engine:
...
def provision(
  ...
  if proj_num:
    sa_email = (
        f"service-{proj_num}@gcp-sa-discoveryengine.iam.gserviceaccount.com"
    )
    print(
        f"  ✓ Ensuring Discovery Engine SA ({sa_email}) has"
        " storage.objectViewer on bucket ..."
    )
  ...
  print(
      f"[3/4] Creating Vertex AI Search Data Store: {data_store_id}"
      f" ({location}) ..."
  )
  ...
  data_store = discoveryengine_v1.DataStore(
      ...
      solution_types=[
          discoveryengine_v1.DataStore.SolutionType.SOLUTION_TYPE_SEARCH
      ],
  ...
  print(
      "Next step -> Ingest documents: uv run python rag/ingest-docs.py"
...
def destroy(
  ...
  print(
      "Cleaning up Vertex AI Search & GCS resources for project"
      f" {project_id} ..."
  )
  parent = f"projects/{project_id}/locations/{location}/collections/default_collection"
...
def main():
  parser = argparse.ArgumentParser(
      description="Provision or Destroy Vertex AI Search Track A resources"
  )
  ...
  parser.add_argument(
      "--data-store-id",
      default=os.getenv("VERTEX_AI_DATA_STORE_ID", "hr-policies-lab-store"),
  )
  parser.add_argument(
      "--engine-id",
  ...
  parser.add_argument(
      "--destroy",
      action="store_true",
      help="Destroy resources instead of provisioning",
  )
  args = parser.parse_args()

  if not args.project:
    print(
        "[ERROR] --project or GOOGLE_CLOUD_PROJECT environment variable is"
        " required."
    )
    sys.exit(1)
