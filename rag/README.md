...
> 💸 **Cost:** Vertex AI Search is a billable, enterprise-tier service. **Clean
> up when you finish the lab** (Step 4 below).
...
-   A GCP project with billing enabled, and the Discovery Engine API enabled:
    ...
    ```
-   Set `GOOGLE_CLOUD_PROJECT`, `VERTEX_AI_DATA_STORE_ID`,
    `VERTEX_AI_SEARCH_ENGINE_ID` in your `.env` (defaults match the setup
    defaults).
...
You can provision the infrastructure using **either** the Python script
(recommended if Terraform is not installed) **or** Terraform:
...
If `terraform` is not installed on your machine, install the standalone binary
in one command:
