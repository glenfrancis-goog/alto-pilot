...
Sanity-check the RAG data store before wiring it into the agent: confirm
semantic
retrieval quality and that citation links come back.

Live mode:
    uv run python rag/verify-rag-search.py --query "outpatient sick leave"
    # uses GOOGLE_CLOUD_PROJECT / VERTEX_AI_SEARCH_ENGINE_ID from the
    environment
...
MOCK_DATABASE = {
    "sick": {
        "title": (
            "Handbook Section 1.1 — Outpatient Sick & Hospitalization Leave"
        ),
        "snippet": (
            "Eligible employees receive up to 14 days of paid outpatient sick"
            " leave per calendar year at 100% of base salary, plus an"
            " additional 46 work days of hospitalization leave. If you are sick"
            " for more than two work days, submit an MC via WorkWeek within 48"
            " hours."
        ),
        "link": (
            "https://hr-portal.altostrat.com/handbook#1.1-sick-hospitalization-leave"
        ),
    },
    "vacation": {
        "title": "Handbook Section 1.2 — Paid Vacation Leave",
        "snippet": (
            "Accrual tiers: 1-6 years = 20 days, 7-10 years = 21 days, 11+"
            " years = 22 days. Shift workers book by actual shift hours; a"
            " vacation day is an 8-hour block, so a 12-hour shift requires 1.5"
            " vacation days."
        ),
    ...
    "ramp": {
        ...
        "snippet": (
            "After at least 10 consecutive weeks of parental/baby-bonding"
            " leave, take up to 2 weeks of paid ramp-back time, working a"
            " minimum of 50% of normal weekly hours while receiving 100% of"
            " normal salary."
        ),
    ...
    "host": {
        ...
        "snippet": (
            "Staying with a friend or relative allows a host gift of up to US"
            " $50 per day with valid receipts. Cash or gift-card host gifts are"
            " strictly prohibited."
        ),
        "link": (
            "https://hr-portal.altostrat.com/handbook#4.3-lodging-transportation"
        ),
    },
    "salon": {
        "title": "Handbook Section 5.2 / 14 — Business Courtesies",
        "snippet": (
            "Business courtesies must never involve gambling, adult"
            " entertainment (strip clubs, hostess bars, room salons), cash, or"
            " cash equivalents (gift cards). Prohibited regardless of value."
        ),
        "link": (
            "https://hr-portal.altostrat.com/handbook#5.2-gifts-entertainment"
        ),
    },
}
...
def query_vertex_search(project_id, location, engine_id, query_text):
  ...
  try:
    ...
    print(
        ...
        " roles/discoveryengine.viewer; engine id."
    )
    sys.exit(1)
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(
      description="Vertex AI Search verification client"
  )
  parser.add_argument("--project", default=os.getenv("GOOGLE_CLOUD_PROJECT"))
  parser.add_argument(
      "--location", default=os.getenv("VERTEX_AI_SEARCH_LOCATION", "global")
  )
  parser.add_argument(
      "--engine-id",
      default=os.getenv("VERTEX_AI_SEARCH_ENGINE_ID", "hr-policies-lab-engine"),
  )
  parser.add_argument("--query", required=True)
  parser.add_argument(
      "--mock",
      action="store_true",
      help="run offline with the built-in mock corpus",
  )
