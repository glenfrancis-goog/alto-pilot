...
Two grading layers:
  ...
  1. FLOOR (default, fast, free): deterministic checks. Factual cases must
  contain
     all `expected_substrings`; refusal cases must contain a refusal phrase.
     This is
  ...
  2. JUDGE (--judge on): an LLM grades each answer against the rubric in
     policy_eval.json across several dimensions (0/1/2), producing a score /100
     and
     a run-over-run delta so you can watch the score climb. See
     evals/RUBRICS.md.
...
def load_agent(target: str):
  ...
  if target == "solution":
    ...
    if root_agent is None:
      sys.exit(
          "agent/agent.py root_agent is None — implement it, or use --target"
          " solution."
      )
...
def evidence_to_str(evidence: list, limit: int = 8000) -> str:
  ...
  for e in evidence:
    ...
    parts.append(
        f"[tool: {e.get('tool')}] {json.dumps(payload, default=str)[:limit]}"
    )
  return "\n\n".join(parts)[: limit * 2]
...
def build_judge_prompt(case, rubric, answer, evidence_str):
  ...
  src_line = (
      ...
      " answer should cite the handbook section it used. Any of these handbook"
      f" sections is an acceptable citation: {srcs}. The handbook repeats some"
      " topics across sections, so ANY section that genuinely supports the"
      " answer counts; a missing, wrong, or fabricated citation does not.\n"
      if srcs
      else ""
  )
...
def judge_case(case, rubric, answer, evidence_str, model, n=1):
  ...
  for _ in range(n):
    ...
    runs.append(scores)
  median = {
      d: int(statistics.median([r[d] for r in runs]))
      for d in case["dimensions"]
  }
...
def score_case(case, rubric, dim_scores):
  ...
  if dim_scores.get("grounding") == 0:
    pct = min(
        pct, rubric.get("gates", {}).get("grounding_zero_caps_case_at", 0.40)
    )
  return pct
...
def run_suite(cases, rubric, runner, use_judge, judge_model, n):
  ...
  for c in cases:
    ...
    if use_judge:
      try:
        dim_scores, why = judge_case(
            c, rubric, answer, evidence_to_str(evidence), judge_model, n
        )
        row["dims"] = dim_scores
        row["why"] = why
...
def print_report(results, use_judge, mode, target, rubric):
  ...
  if not use_judge:
    for r in results:
      if "error" in r:
        ...
        print(
            f"[{'PASS' if r['floor_pass'] else 'FAIL'}] {r['id']} —"
            f" {r['floor_why']}"
        )
  ...
  hdr = (
      "case".ljust(30) + "".join(d[:4].rjust(6) for d in dim_order) + "   case%"
  )
  print("\n" + hdr)
  print("-" * len(hdr))
  for r in results:
    if "error" in r:
      ...
      continue
    cells = "".join(
        (str(r["dims"][d]) if d in r.get("dims", {}) else "-").rjust(6)
        for d in dim_order
    )
  ...
  print("-" * len(hdr))
  print(
      f"{'TOTAL'.ljust(30)}{''.join(' ' * 6 for _ in dim_order)}   {total:5.1f}"
      " / 100"
  )
  ...
  if s1:
    print(
        "⚠  SUSPECT (judge high but floor failed — check phrasing/grader):"
        f" {s1}"
    )
  s2 = [
      r["id"]
      for r in scored
      if r.get("floor_pass") and r["dims"].get("grounding") == 0
  ]
  if s2:
    print(
        "⚠  SUSPECT (floor passed but GROUNDING=0 — likely"
        f" hardcoded/ungrounded): {s2}"
    )
  ...
  if hard:
    ...
    print(
        f"BADGE (>= {int(thr*100)}% on hard cases {hard}): "
...
def show_delta_and_save(summary, mode, target):
  ...
  if prev:
    ...
    print(
        f"\nDELTA vs last {key} run: {prev.get('total', 0):.1f} ->"
        f" {summary['total']:.1f}  ({d:+.1f}) {arrow}"
    )
    regressions = [
        cid
        for cid, p in summary["per_case"].items()
...
def main():
  ...
  ap.add_argument(
      "--mode", choices=["okf", "rag", "hybrid"], help="override RETRIEVAL_MODE"
  )
  ...
  ap.add_argument(
      "--judge-model", default=os.getenv("EVAL_JUDGE_MODEL", "gemini-3.5-flash")
  )
  ap.add_argument(
      "--self-consistency",
      type=int,
      default=1,
      help="judge N times, take median",
  )
  ap.add_argument(
      "--compare-modes",
      action="store_true",
      help="run okf and rag side by side",
  )
  args = ap.parse_args()
  ...
  modes = (
      ...
      else [args.mode or os.getenv("RETRIEVAL_MODE", "okf")]
  )
  summaries = {}
  for mode in modes:
    ...
    print(
        f"\n===== mode={mode} | target={args.target} | judge={args.judge} |"
  ...
  if use_judge and args.compare_modes and len(summaries) == 2:
    ...
    print(
        f"{'TOTAL'.ljust(33)} okf {summaries['okf']['total']:5.1f}   rag"
        f" {summaries['rag']['total']:5.1f}"
    )
